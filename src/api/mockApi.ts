...
const ORDERS: Order[] = [
  ...
  {
    id: "o_10352",
    orderNumber: "SP-10352",
    placedAt: "2026-05-02",
    status: "delivered",
    items: [
      {
        id: "i_9",
        name: "Ridge Merino Beanie",
        unitPriceCents: c(34),
        quantity: 3,
        returnEligible: true,
      },
  ...
  {
    id: "o_10341",
    orderNumber: "SP-10341",
    placedAt: "2026-04-24",
    status: "shipped",
    items: [
      {
        id: "i_11",
        name: "Harbor Rain Jacket",
        unitPriceCents: c(158),
        quantity: 1,
        thumbColor: "#E7EEFF",
        returnEligible: true,
      },
  ...
  {
    id: "o_10330",
    orderNumber: "SP-10330",
    placedAt: "2026-04-15",
    status: "delivered",
    items: [
      {
        id: "i_12",
        name: "Kettle & Stone Mug Set",
        unitPriceCents: c(46),
        quantity: 1,
        thumbColor: "#EFEAE2",
        returnEligible: true,
...
const latency = () =>
  apiConfig.minLatencyMs + Math.random() * (apiConfig.maxLatencyMs - apiConfig.minLatencyMs);
...
export async function listOrders(params: ListParams = {}): Promise<Page<Order>> {
  ...
  if (Math.random() < apiConfig.listFailureRate)
    throw new ApiError(503, "The orders service is temporarily unavailable.");
...
export async function submitReturn(req: ReturnRequest): Promise<ReturnReceipt> {
  await wait(latency());
  if (Math.random() < apiConfig.submitFailureRate)
    throw new ApiError(500, "We couldn't submit your return. Please try again.");
  if (req.items.length === 0) throw new ApiError(422, "Select at least one item to return.");
  return {
    returnId: "rma_" + Math.random().toString(36).slice(2, 8),
